...
import { MapPin, Car, Clock, Loader2, AlertTriangle, Edit, XCircle, DollarSign, MessageSquare, CheckCheck, PlusCircle, Info } from "lucide-react";
...
export default function MyActiveRidePage() {
 ...
 const handleEditSuggestionClickFactory = useCallback((formFieldNameOrStopIndex: 'pickupLocation' | 'dropoffLocation' | number) => (suggestion: google.maps.places.AutocompletePrediction, formOnChange: (value: string) => void) => {
  ...
  const vehicleTypeDisplay = (activeRide?.vehicleType as string) || 'Vehicle N/A';
  const statusDisplay = activeRide?.status ? activeRide.status.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Status N/A';
  const pickupAddressDisplay = (activeRide?.pickupLocation && typeof activeRide.pickupLocation === 'object' && 'address' in activeRide.pickupLocation) ? String(activeRide.pickupLocation.address) : 'Pickup N/A';
  const dropoffAddressDisplay = (activeRide?.dropoffLocation && typeof activeRide.dropoffLocation === 'object' && 'address' in activeRide.dropoffLocation) ? String(activeRide.dropoffLocation.address) : 'Dropoff N/A';
